function Asset(props) {
  ...
  return (
    <article
      className="asset"
      id={article.name.split(" ").join("_").toLowerCase()}
    >
      <div>
        {article.icon ? (
          <img src={article.icon} />
        ) : article.type == "website" || article.type == "software" ? (
          <img
            src={
              article.homepage.split("/")[0] +
              "//" +
              article.homepage.split("/")[2] +
              "/favicon.ico"
            }
          />
        ) : (
      ...
      <div>
        ...
        {article.homepage ? (
          <a href={article.homepage} target="_blank">
            View page
          </a>
        ) : (
          ""
